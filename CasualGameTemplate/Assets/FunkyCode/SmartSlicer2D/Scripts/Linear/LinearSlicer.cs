...
namespace Slicer2D {
	public class LinearSlicer {
		...
		static private Slice2D SingleSlice(Polygon2D polygon, Pair2D slice) {
			...
			bool vertexToVertex = SliceVertexToVertex.Get(polygon, slice);

			if (vertexToVertex == true) {
				return(SliceVertexToVertex.Slice(polygon, slice));
			}

			Vector2D vertexToIntersection = SliceVertexToIntersection.Get(polygon, slice);

			if (vertexToIntersection != null) {
				return(SliceVertexToIntersection.Slice(polygon, slice, vertexToIntersection));
			}
		...
		public class SliceVertexToVertex {
			static public Slice2D Slice(Polygon2D polygon, Pair2D slice) {
				...
				if (vertexCollisionCount > 0) {
					Debug.LogWarning("Slicer2D: unfiltered vertex collision: " + vertexCollisionCount);
				}
...
/*
				if (collisionCount == 3) {  // Complicated Slices With Holes
					if (Slicer2D.Debug.enabled) {
